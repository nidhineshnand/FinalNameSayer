...
public abstract class NameSayerFileList {
    ...
    private void normalizeAudio(){
        try {
            ProcessBuilder builder = new ProcessBuilder("bash", "-c", "ffmpeg -i ./resources/concatenations/output1.wav -filter:a \"dynaudnorm\" ./resources/concatenations/output2.wav");
            Process process = builder.start();
            process.waitFor();
            //Exception handling for the process builder
        } catch (IOException ioe) {
